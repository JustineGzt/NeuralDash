...
export const Register = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Échec de l\'inscription avec Google';
  ...
  return (
    <div className="min-h-screen bg-[#03070b] text-cyan-200 flex items-center justify-center px-4 py-8">
      {/* Effets de fond */}
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="border border-cyan-500/30 rounded-3xl bg-[#050b12]/90 backdrop-blur-xl p-8 shadow-[0_0_60px_rgba(0,0,0,0.8)]">
          ...
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-cyan-400 mb-2">INSCRIPTION</h1>
            <div className="h-1 w-20 bg-gradient-to-r from-cyan-500 to-blue-500 mx-auto rounded-full" />
            <p className="mt-4 text-sm text-cyan-200/60 uppercase tracking-wider">Créer un nouveau compte</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label htmlFor="password" className="block text-xs uppercase tracking-wider text-cyan-200/80 mb-2">
                Mot de passe
              </label>
              <input
                id="password"
                ...
                placeholder="••••••••"
              />
              <p className="mt-1 text-xs text-cyan-200/40">Minimum 6 caractères</p>
            </div>

            <div>
              <label htmlFor="confirmPassword" className="block text-xs uppercase tracking-wider text-cyan-200/80 mb-2">
            ...
            <div className="flex items-start gap-3">
              <input
                ...
                className="mt-1 w-4 h-4 rounded bg-black/40 border border-cyan-500/30 text-cyan-500 focus:ring-2 focus:ring-cyan-400/20 cursor-pointer"
                required
              />
              <label htmlFor="acceptTerms" className="text-xs text-cyan-200/80 leading-relaxed cursor-pointer">
                J'accepte les{' '}
                <Link 
                  to="/terms" 
                  target="_blank"
